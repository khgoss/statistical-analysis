...
class WindowLv1_Miner(wx.Frame):
	def __init__(self, title):
		super(WindowLv1_Miner,self).__init__(parent=None,title=title,size=(1000,600))
		self.file_path = None
		self.result_path = None
		self.dataset = None
		self.paired = False
	...
	def display_window(self):
		...
		button_1.Bind(wx.EVT_BUTTON,self.select_filepath)
		self.text_1=wx.StaticText(panel,label='Files for the same experimental group should be stored in the same subfolder.',
			style=wx.ALIGN_LEFT|wx.ST_ELLIPSIZE_END)
		module_1.Add(button_1,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		module_1.Add(self.text_1,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,10,0)
		...
		module_2=wx.BoxSizer(wx.HORIZONTAL)
		button_2=wx.Button(panel,label='Select the folder containing the files for the \ncontrol group',size=(300,40))
		button_2.Bind(wx.EVT_BUTTON, self.select_control)
		self.text_2=wx.StaticText(panel,label='Only select a control group if you are comparing it to at least 2 other groups.',
			style=wx.ALIGN_LEFT|wx.ST_ELLIPSIZE_END)
		module_2.Add(button_2,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		module_2.Add(self.text_2,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(module_2,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		...
		module_3=wx.BoxSizer(wx.HORIZONTAL)
		button_3=wx.Button(panel,label='Is the data paired?',size=(300,40))
		button_3.Bind(wx.EVT_BUTTON,self.input_paired)
		module_3.Add(button_3,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(module_3,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0) 

		module_4=wx.BoxSizer(wx.HORIZONTAL)
		button_4=wx.Button(panel,label='Specify how much information to print',size=(300,40))
		button_4.Bind(wx.EVT_BUTTON,self.input_printAll)
		module_4.Add(button_4,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(module_4,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0)

		module_5=wx.BoxSizer(wx.HORIZONTAL)
		button_5=wx.Button(panel,label='Select the folder that stores\nthe results',size=(300,40))
		button_5.Bind(wx.EVT_BUTTON,self.select_result_path)
		module_5.Add(button_5,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(module_5,0,wx.LEFT|wx.RIGHT|wx.EXPAND,10)
		boxsizer.Add(0,5,0)

		button_6=wx.Button(panel,label='Start to mine data',size=(300,40))
		button_6.Bind(wx.EVT_BUTTON,self.mine_data)
		boxsizer.Add(button_6,0,wx.RIGHT|wx.ALIGN_RIGHT,90)
		boxsizer.Add(0,10,0)
	...
	def input_printAll(self, event):
		dialog=wx.MessageDialog(self,'Print all statistical analysis (YES) or only significant findings (NO)?','Print all?', wx.YES_NO|wx.ICON_QUESTION)
		if dialog.ShowModal()==wx.ID_YES:
			self.printAll=True
		else:
			self.printAll=False
	...
	def select_filepath(self, event): #select folder containing Excel files of data
		...
		if dialog.ShowModal()==wx.ID_OK:
			self.file_path=dialog.GetPath()
			self.text_1.SetLabel('Path to data files: '+self.file_path)
		dialog.Destroy()

	def select_control(self, event):
		dialog=wx.DirDialog(self,'Select a directory','',style=wx.DD_DEFAULT_STYLE)
		if dialog.ShowModal()==wx.ID_OK:
			control_path=dialog.GetPath()
			self.text_1.SetLabel('Path to control group files: '+control_path)
		else:
			return
		dialog.Destroy()
		self.control = self.read_folder(control_path)

	def select_result_path(self, event):
		dialog=wx.DirDialog(self,'Select a directory','',style=wx.DD_DEFAULT_STYLE)
		if dialog.ShowModal()==wx.ID_OK:
			self.result_path=dialog.GetPath()
			self.text_1.SetLabel('Path to data files: '+self.result_path)
		dialog.Destroy()
		...
		for file in os.listdir(self.file_path):
			...
			if os.path.isdir(subfolder):
				...
				if folder_data != None:
					...
					print("Folder", subfolder, "does not contain the proper data files.")
		self.dataset = data


	def mine_data(self, event):
		self.read_all_folders()
		DM = statistical_analysis.data_mining(self.dataset, self.control, self.paired, self.printAll, self.result_path)
...
class InitialWindow(wx.Frame):
	...
	def dispaly_window(self):
		...
		button5 = wx.Button(panel, label='Mine Data')
		boxsizer.Add(button5,0,wx.LEFT|wx.RIGHT|wx.EXPAND,40)
		button5.Bind(wx.EVT_BUTTON,self.data_mining)
		boxsizer.Add(0,30,0)
	...
	def data_mining(self, event):

		WindowLv1_Miner('Mine Data')
